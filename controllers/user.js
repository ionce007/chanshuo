...
async function refreshToken(req, res) {
  ...
  if (user) {
    await user.update({ accessToken: uuid });
    res.json({ status: true, message: 'ok', accessToken: uuid });
  } else {
    res.json({ status: false, message: '用户不存在' });
  }
}
