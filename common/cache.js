...
const { PAGE_STATUS, PAGE_TYPES, FORMULA_KIND, FORMULA_CATEGORY } = require('./constant');
...
async function loadAllPages() {
  ...
  try {
    pages = await Page.findAll({
      where: { [Op.or]: [{ pageStatus: PAGE_STATUS.PUBLISHED },{ pageStatus: PAGE_STATUS.TOPPED }] },
      order: [ ['pageStatus', 'DESC'], ['updatedAt', 'DESC'] ],
      raw: true
    });
    updateId2Index();
  } catch (e) {
...
async function getFormulasByRange(start, num) {
  ...
  if (num === -1) {
    return formulas.slice(start);
  }
  return { total: formulas.length, data: formulas.slice(start, start + num) };
}
async function loadAllFormulas() {
  // The password & token of user shouldn't be load!
  try {
    formulas = await Formula.findAll({
      where: { [Op.and]: [{ isSell: 1 }, { isdir: 0 }] },
      order: [['lastupdate', 'DESC']],
      raw: true
    });
    updateFormulaId2Index();
  } catch (e) {
    console.log('Failed to load all formula!');
    console.error(e);
  }
}

let formulaId2index = new Map();

function updateFormulaId2Index() {
  formulaId2index.clear();
  for (let i = 0; i < formulas.length; i++) {
    formulaId2index.set(formulas[i].fs_id, i);
  }
}
