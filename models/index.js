...
async function initializeDatabase() {
  ...
  const isNoAdminExisted = (await User.findOne({ where: { isAdmin: true } })) === null;
  if (isNoAdminExisted) {
    console.log('No admin user existed! Creating one for you.');
    await User.create({username: 'admin',password: hashPasswordWithSalt('123456'),displayName: 'Administrator',isAdmin: true,isModerator: true});
  }
