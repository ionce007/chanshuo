...
import {Table, Tag, Button, message as Message, Tooltip, Space, Popconfirm, Input, Select, Row, Col,} from 'antd';
    ...
    this.columns = [
      {
        ...
        render: (value, record) => (
          ...
          >
            <span>
              <a target='_blank' rel="noreferrer" href={'/page/' + record.link}>{value ? value : '无标题'}</a>
            </span>
          </Tooltip>
        ),
      },
      ...
      },
      {
        ...
        render: (record) => (
          <Space>
            <Button type="primary" onClick={() => this.editPage(record.id)}>编辑</Button>
            <Button onClick={() => this.exportPage(record.id)}>导出</Button>
            <Popconfirm  placement="rightTop" title={'确认删除页面？'} onConfirm={() => this.deletePage(record.id)} okText="确认" cancelText="取消">
    ...
    return (
      <div className={'content-area'}>
        ...
        <Col span={2} style={{ marginRight: '8px' }}>
            <Button type="primary" onClick={() => this.newArticle()} >创建文章</Button>
          </Col>
          <Col span={2} style={{ marginRight: '8px' }}>
            <Select
